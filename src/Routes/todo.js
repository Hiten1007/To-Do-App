...
export const todoapp = (() => {
  ...
  function createTaskElement(task) {
    ...
    checkBox.addEventListener("change", () => {
      ...
      tasks = tasks.filter(
        (todo) => todo.dueDate !== task.dueDate && todo.title !== task.title
      );
  ...
  const displayTasks = (activeProject) => {
    ...
    if (activeProject === "Today") {
      ...
      todayTasks.length !== 0 && todayTasks
        ? displayAll(todayTasks)
        : (tasksContainer.textContent = "No tasks today");
    } else if (activeProject === "Next 7 Days") {
      const next7Days = time.getNext7Days();
      next7Days.length !== 0 && next7Days
        ? displayAll(next7Days)
      ...
      projectTasks.length !== 0 && projectTasks
        ...
        : (tasksContainer.textContent = "No tasks");
    }
